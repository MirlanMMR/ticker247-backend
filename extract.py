    ...
    def apply(self, text: str) -> str:
        ...
        for m in self._SEAM.finditer(body[:400]):
            ...
            if re.search(r"[.!?…]", prefix):
                continue
            # Шов внутри незакрытой скобки — не стык двух текстов, а
            # обычная вставка вроде «(ям Беккари)»: строчное слово перед
            # скобкой и имя с большой буквы внутри неё дают тот же рисунок,
            # что и настоящий повтор лида. Kaktus, 22.09.2026: «269
