...
class AgencyLeadSanitizer(BaseSanitizer):
    ...
    _RX = re.compile(
        r"(?:^|\n)\s*([А-ЯЁ][А-ЯЁ \-]{2,30}|[A-Z][A-Z \-]{2,30}),\s*"
    ...
    def apply(self, text: str) -> str:
        ...
        if not m:
            return text
        start = m.start()
        # Зачин в самом начале — обычная заметка, ничего лишнего нет
        if start < 20:
            return text
        tail = text[start:].strip()
        # Не оставляем читателя с огрызком: если после зачина почти ничего,
        # значит зачин не тот, что мы думали
        return tail if len(tail) >= 200 else text
