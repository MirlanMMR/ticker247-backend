...
def filter_with_gemini(news_list, lang="ru"):
    ...
    prompt = f"""Ты редактор новостного агрегатора Ticker 24/7 для аудитории: {REGIONAL_SPORT_PRIORITY.get(lang, "глобальной")}.
Отбери из списка только то, что эта аудитория реально хочет читать прямо сейчас.

═══ ЖЁСТКИЕ ФИЛЬТРЫ — удалять всегда, без исключений ═══

ЯЗЫК: Если заголовок написан на японском, корейском, китайском, арабском или другом языке,
не соответствующем аудитории пула — удалять. Исключение: если это новостная статья
на языке пула о событии в другой стране.

РАЗВЛЕКАТЕЛЬНЫЙ КОНТЕНТ НЕ ДЛЯ МАССОВОЙ АУДИТОРИИ:
- Аниме, манга, k-pop, j-pop, дорамы — нишевый контент, не для массовой аудитории
- Музыкальные клипы, тизеры, трейлеры сериалов/фильмов — реклама, не новость
- Видеоигры, стриминг, косплей
- Блогеры, инфлюенсеры, ютуберы как тема

МУСОР:
- Бюрократия без последствий: протоколы, меморандумы, плановые заседания
- Реклама и PR под видом новости
- Исторические материалы без актуального повода
- Кликбейт без содержания
- Дубликаты (оставь одну лучшую версию)

═══ ЧТО БРАТЬ ═══

РЕГИОНАЛЬНЫЙ ПРИОРИТЕТ (аудитория: {REGIONAL_SPORT_PRIORITY.get(lang, "глобальная")}):
События, происходящие в этом регионе, важнее аналогичных в других местах:
- Происшествия, ЧС, аварии с жертвами = priority 2
- Политика: выборы, отставки, скандалы, аресты = priority 2
- Спорт: победы и медали местных спортсменов, крупные местные турниры = priority 2
- Экономика: цены, курсы, закрытия, дефицит = priority 1-2
- Общество: эпидемии, отравления, коммунальные проблемы = priority 2

МИРОВАЯ ПОВЕСТКА (для всех пулов):
- Геополитические кризисы, войны, катастрофы = priority 2
- Крупный спорт в разгаре (финалы, рекорды, сенсации мирового уровня) = priority 1-2
- Технологии: массовые анонсы и релизы = priority 1
- Культура: кино, музыка, театр которые обсуждает массовая аудитория = priority 0-1
...
═══ ПРИОРИТЕТЫ ═══
priority=2: ЧС, теракты, геополитические кризисы, победы своего региона
priority=1: важные новости, крупные турниры, значимые события
