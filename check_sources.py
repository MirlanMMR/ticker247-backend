...
только если не отвечает 20% источников и больше — обычная недельная
норма (2-6%) через это спокойно проходит, а настоящая авария (сеть,
ключи, блокировка раннера) — нет. Отчёт по единичным сбоям всё равно
печатается каждый раз, просто без exit 1.
"""
import sys
import requests
