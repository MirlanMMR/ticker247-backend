...
CANDIDATES = [
    ...
    # украинская речь ушла бы в русский пул. Настоящий канал русской
    # редакции — «Радио Свобода Live» — уже круглосуточный и 1080p,
    # добавлен сразу в LIVE_CHANNELS, в опросе кандидатов не нуждается.
    ("@theinsider", "The Insider", "ru"),
    ("@belsat", "Белсат", "ru"),
    ("@nexta_tv", "NEXTA", "ru"),
    # СНГ, русскоязычные каналы (не местный язык — иначе эфир попадёт
    # ВСЕМУ русскому пулу, а не только своей стране). Проверено 03.09.2026:
    # 24KZ (Казахстан) — эфир идёт, 1080p, подтверждено yt-dlp сразу.
    # Беларусь 24 — канал жив (код 200), при двух проверках подряд не в
    # эфире; ОНТ, СТВ, БелТА заблокированы Google 03.04.2026 целиком, их не
    # проверяем вовсе. Копим расписание, как для остальных кандидатов.
    ("@tv24kz", "24KZ", "ru"),
